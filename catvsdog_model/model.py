...
def create_model(input_shape, optimizer, loss, metrics):
    ...
    x = keras.layers.Rescaling(1. / config.model_configs.scaling_factor)(x)
...
# Create model
classifier = create_model(input_shape = config.model_configs.input_shape, 
                          optimizer = config.model_configs.optimizer, 
                          loss = config.model_configs.loss, 
                          metrics = [config.model_configs.accuracy_metric])
